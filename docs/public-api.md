...
The public API lets you drive your IMCRM instance from your own
...
```
Authorization: Bearer imcrm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** IMCRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer imcrm_live_xxx"
```
...
`broadcasts:send`. The broadcast, recipient rows, and one durable
delivery job per recipient are persisted atomically, so the call
returns fast. A scheduled worker claims and retries those jobs; poll
`GET /api/v1/broadcasts/{id}` for progress.

```bash
curl -X POST https://your-crm.example.com/api/v1/broadcasts \
  -H "Authorization: Bearer imcrm_live_xxx" \
  -H "Idempotency-Key: july-promo-2026" \
  -H "Content-Type: application/json" \
  -d '{
        "name": "July promo",
...
Invalid phone numbers are dropped and counted as `rejected`. Contacts
that opted out of WhatsApp marketing are not sent; they are recorded
as `skipped` rather than technical failures. Reuse `Idempotency-Key`
when retrying the same request. Response (202):
...
`sending` to `sent`; `delivered_count` / `read_count` keep climbing as
Meta delivery webhooks arrive. `skipped_count` exposes consent
suppression separately from `failed_count`. `404` for another account's
broadcast.

The deployment must schedule
`GET /api/whatsapp/broadcast/cron` with the `x-cron-secret` header set
to `AUTOMATION_CRON_SECRET`. Each invocation drains a bounded batch;
`BROADCAST_CRON_BATCH_SIZE` optionally changes the default of 20
(maximum 100).
...
Rather than polling, register an endpoint and IMCRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; IMCRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer imcrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/imcrm", "events": ["message.received"] }'
...
Headers: `X-Imcrm-Event`, `X-Imcrm-Webhook-Id`, and `X-Imcrm-Signature`.

### Verifying the signature

`X-Imcrm-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages IMCRM stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/igor-marcelloDEV/IMCRM/issues/245). Future ideas
