...
**[IMCRM](https://github.com/igor-marcelloDEV/IMCRM)** — the self-hostable
...
It's a thin wrapper over IMCRM's public [`/api/v1`](../docs/public-api.md)
REST API. All auth, scoping, and rate limiting are enforced by your
IMCRM instance — this server just exposes the API as MCP tools.

## Prerequisites

1. A running IMCRM instance (your own self-hosted deploy).
...
| ------------------------- | -------- | -------------------------------------------------------------- |
| `IMCRM_BASE_URL`          | yes      | Your instance URL, e.g. `https://crm.example.com`              |
| `IMCRM_API_KEY`           | yes      | An API key from the dashboard                                  |
| `IMCRM_ENABLE_WRITES`     | no       | `true` to expose contact writes + message sending             |
| `IMCRM_ENABLE_BROADCASTS` | no       | `true` to expose mass broadcasts (needs `IMCRM_ENABLE_WRITES`) |
...
"env": {
  "IMCRM_BASE_URL": "https://crm.example.com",
  "IMCRM_API_KEY": "imcrm_live_xxxxxxxxxxxxxxxxxxxxxxxx",
  "IMCRM_ENABLE_WRITES": "true",
  "IMCRM_ENABLE_BROADCASTS": "true"
}
```
...
1. **Read-only by default.** Write and broadcast tools are not even
   ...
   `IMCRM_ENABLE_WRITES` / `IMCRM_ENABLE_BROADCASTS`.
2. **API-key scopes.** Whatever the guards allow, your IMCRM instance
