...
async function recomputeOrderTotal(dealId: string, orderId: string) {
  ...
  const { data: items } = await db.from("order_items").select("total_cents").eq("order_id", orderId);
  const total = ((items as Array<{ total_cents: number }> | null) ?? []).reduce((s, i) => s + i.total_cents, 0);
  await db.from("orders").update({ subtotal_cents: total, total_cents: total }).eq("id", orderId);
  // Keep the Deal's value in lockstep — it's the number shown on the
  // kanban card and everywhere else in the pipeline UI.
  await db.from("deals").update({ value: total / 100 }).eq("id", dealId);
  return total;
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!deal.contact_id) {
    return NextResponse.json({ error: "Este negócio não tem um contato vinculado" }, { status: 400 });
  }
  ...
  const { data: item } = await db
    ...
    .eq("is_active", true)
    .maybeSingle();
  if (!item) return NextResponse.json({ error: "Item do catálogo não encontrado" }, { status: 404 });

  let { data: order } = await db.from("orders").select("id").eq("deal_id", dealId).maybeSingle();
  if (!order) {
    const { data: created, error } = await db
      ...
      .maybeSingle();
    if (error || !created) {
      return NextResponse.json({ error: error?.message ?? "Não foi possível criar o pedido" }, { status: 500 });
    }
    order = created;
  }
