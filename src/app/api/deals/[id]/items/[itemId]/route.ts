...
async function loadLine(dealId: string, itemId: string) {
  const db = supabaseAdmin();
  const { data: order } = await db.from("orders").select("id, account_id").eq("deal_id", dealId).maybeSingle();
...
export async function PATCH(
  ...
  const db = supabaseAdmin();
  const { data: deal } = await db.from("deals").select("id").eq("id", dealId).eq("account_id", ctx.accountId).maybeSingle();
  ...
  return NextResponse.json({ order_id: found.order.id, items: finalItems ?? [] });
}
