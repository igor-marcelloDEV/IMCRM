...
export async function POST(request: Request) {
  try {
    ...
    if (!rawMessages) {
      return NextResponse.json({ error: "O campo 'messages' é obrigatório" }, { status: 400 })
    }
    ...
    if (!config) {
      return NextResponse.json(
        {
          error: 'Nenhum agente configurado ainda. Adicione a chave do seu provedor em Configuração.',
