...
async function processWebhook(body: { entry?: InstagramWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    ...
    if (configError) {
      console.error('[instagram webhook] config lookup failed for', entry.id, configError)
      continue
    }
    if (!config) {
      console.warn('[instagram webhook] no config found for ig account', entry.id)
      continue
    }
    ...
    for (const change of entry.changes ?? []) {
      ...
      await ingestInstagramComment(supabaseAdmin(), accountId, configOwnerUserId, {
        commentId: value.id,
    ...
    for (const event of entry.messaging ?? []) {
      ...
      await ingestInstagramMessage(supabaseAdmin(), accountId, configOwnerUserId, {
