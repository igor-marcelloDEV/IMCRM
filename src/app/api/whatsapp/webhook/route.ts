...
export async function POST(request: Request) {
  ...
  // Persist the exact signed bytes before acknowledging Meta. `after()` is
  // still the fast normal path, but the durable inbox + protected replay
  // cron now cover a runtime crash or transient failure after this 200.
  ...
  try {
    ...
    console.error('[webhook] could not persist inbound request:', error);
    return NextResponse.json(
  ...
  // (a slow ack triggers Meta retries + duplicate inserts).
  //
  ...
  if (shouldProcess) {
    after(async () => {
      try {
        await runTrackedInboundWebhook(db, 'whatsapp', eventKey, () =>
          processWebhook(body)
        );
      } catch (error) {
...
async function processMessage(
  ...
  // Reactions short-circuit before any message-shaped work — they
  // aren't messages, they update `message_reactions` state. Resolve
  // contact/conversation directly here (rather than going through
  // `ingestInboundMessage`, which is message-insert-shaped) so a
  // reaction that's the very first inbound contact from someone still
  // creates the contact/conversation and fires `conversation.created`.
  ...
  // Hand off to the provider-agnostic ingestion pipeline (shared with
  // the Baileys worker webhook) — find/create contact + conversation,
  // persist the message, flag broadcast replies, dispatch Flows /
  // automations / AI auto-reply, fan out message.received.
  await ingestInboundMessage(supabaseAdmin(), accountId, configOwnerUserId, {
    provider: 'meta_cloud_api',
    providerMessageKey: message.id,
    ...
    mediaUrl,
    interactiveReplyId,
    replyToProviderMessageKey: message.context?.id ?? null,
...
async function parseMessageContent(
  ...
  switch (message.type) {
    ...
    case 'interactive': {
      ...
      return { ...empty, contentText: '[Resposta interativa]' };
    }

    default:
      return {
        ...empty,
        contentText: `[Tipo de mensagem não suportado: ${message.type}]`,
      };
  }
}
