...
export default function JoinPage() {
  ...
  const ROLE_LABEL: Record<PeekOk['role'], string> = {
    ...
    agent: t('roleLabel.agent'),
    viewer: t('roleLabel.viewer'),
  };

  const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
    not_found: { title: t('fail.notFoundTitle'), body: t('fail.notFoundBody') },
    used: { title: t('fail.usedTitle'), body: t('fail.usedBody') },
    expired: { title: t('fail.expiredTitle'), body: t('fail.expiredBody') },
    server_error: { title: t('fail.serverErrorTitle'), body: t('fail.serverErrorBody') },
  };
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(payload.error || t('toastConflictDefault'));
        } else {
