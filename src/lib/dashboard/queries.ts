...
/** Minimal shape of next-intl's `t()` — avoids importing next-intl into this client-agnostic module. */
...
export async function loadActivity(db: DB, limit = 20, t: Translator): Promise<ActivityItem[]> {
  ...
  for (const m of (msgs.data ?? []) as unknown as Array<{
    ...
    const who = contact?.name || contact?.phone || t('unknownFallback')
    items.push({
      id: `msg-${m.id}`,
      kind: 'message',
      text: t('newMessageFrom', { who }),
      at: m.created_at,
  ...
  for (const b of (broadcasts.data ?? []) as Array<{
    ...
    const text =
      b.status === 'sent'
        ? t('broadcastSentTo', { name: b.name, count: b.total_recipients })
        : t('broadcastStatus', { name: b.name, status: b.status, count: b.total_recipients })
    items.push({
      id: `broadcast-${b.id}`,
      kind: 'broadcast',
      text,
  ...
  for (const l of (autoLogs.data ?? []) as unknown as Array<{
    ...
    const who = contact?.name || contact?.phone || t('contactFallback')
    const autoName = automation?.name || t('automationFallback')
    items.push({
      id: `auto-${l.id}`,
      kind: 'automation',
      text:
        l.status === 'failed'
          ? t('automationFailedFor', { name: autoName, who })
          : t('automationTriggeredFor', { name: autoName, who }),
      at: l.created_at,
    })
  }
