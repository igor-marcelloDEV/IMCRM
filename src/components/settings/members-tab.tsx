...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('toastLoadMembersFailed'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('toastLoadInvitationsFailed'));
          return;
        }
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('toastUpdateRoleFailed'));
        return;
      }
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('toastRemoveMemberFailed'));
        return;
      }
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('toastRevokeInvitationFailed'));
        return;
      }
                          ...
                          <Avatar className="size-9 shrink-0">
                            {member.avatar_url ? (
                              <AvatarImage
                                ...
                                alt={member.full_name || t('memberAltFallback')}
                              />
                            ) : null}
