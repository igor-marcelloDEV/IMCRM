...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    if (!phoneNumberId.trim()) {
      toast.error(t('toastPhoneNumberIdRequired'));
      return;
    }
    if (!config && (!accessToken.trim() || !tokenEdited)) {
      toast.error(t('toastAccessTokenRequiredInitial'));
      return;
    }
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error(t('toastReenterAccessToken'));
        setSaving(false);
        return;
      }
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          t('toastSavedButRegisterFailed', { reason: data.registration_error }),
          { duration: 12000 },
        );
        ...
        toast.success(
          t('toastCredentialsVerifiedSkipped'),
          { duration: 10000 },
        );
        setPin('');
      } else {
        toast.success(
          data.phone_info?.verified_name
            ? t('toastLiveWithName', { name: data.phone_info.verified_name })
      ...
      if (payload.connected) {
        ...
        toast.success(
          payload.phone_info?.verified_name
            ? t('toastConnectedWithName', { name: payload.phone_info.verified_name })
            : t('toastApiConnectionSuccess')
        );
      } else {
        setConnectionStatus('disconnected');
        setResetReason(payload.needs_reset ? 'token_corrupted' : payload.reason === 'meta_api_error' ? 'meta_api_error' : null);
        setStatusMessage(payload.message || '');
        toast.error(payload.message || t('toastApiConnectionFailed'));
      }
    } catch (err) {
      console.error('Test connection error:', err);
      setConnectionStatus('disconnected');
      toast.error(t('toastConnectionTestFailed'));
    } finally {
      setTesting(false);
    }
  }
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      toast.error(t('toastVerifyEndpointUnreachable'));
    } finally {
  ...
  async function handleReset() {
    ...
    try {
      ...
      toast.success(t('toastConfigCleared'));
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
      setAccessToken('');
  ...
  function handleCopyWebhookUrl() {
    ...
    toast.success(t('toastWebhookUrlCopied'));
  }
