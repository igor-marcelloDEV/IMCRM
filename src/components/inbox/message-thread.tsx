...
export function MessageThread({
  ...
  const handleSend = useCallback(
    async (text: string, replyToId?: string) => {
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : t("networkErrorFallback");
        toast.error(t("toastFailedSend", { reason }));
        onUpdateMessage(tempId, { status: "failed" });
      }
    },
  ...
  const handleAssignChange = useCallback(
    async (agentId: string | null) => {
      ...
      if (error) {
        ...
        toast.error(t("toastFailedUpdateAssignment"));
        return;
      }
  ...
  const displayName = contact.name || contact.phone || contact.instagram_username || contact.id;
