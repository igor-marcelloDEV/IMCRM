-- ============================================================
-- 088_checkout_picks_delivery_pickup_pipeline.sql
--
-- create_public_store_order (065, rewritten by 084) picked "the
-- account's first pipeline by created_at" for every new deal,
-- ignoring p_fulfillment_type entirely. In practice this self-healed
-- one statement later — the function's own `UPDATE orders SET
-- deal_id = v_deal_id` re-fires sync_order_to_pipeline() (087), whose
-- UPDATE branch already re-routes the deal by fulfillment_type — so
-- the row lands correctly by the time the transaction commits, but
-- only by accident of a later, unrelated statement. That's a trap for
-- the next person reading this function top to bottom.
--
-- CREATE OR REPLACE with the exact same signature — this is the same
-- function object (permissions/grants carry over), just with the
-- pipeline lookup made explicit and correct up front instead of
-- relying on the trigger's second pass.
-- ============================================================
...
BEGIN
  ...
  -- Same funnel a completed checkout will end up in via
  -- sync_order_to_pipeline() (087) — picked here directly so the
  -- deal is filed correctly from its very first row, not fixed up by
  -- that trigger firing again later in this same function.
  v_pipeline_kind := CASE p_fulfillment_type
    WHEN 'delivery' THEN 'orders_delivery'
    WHEN 'pickup' THEN 'orders_pickup'
    ELSE 'orders'
  END;
  v_pipeline_id := public.ensure_order_pipeline(p_account_id, v_pipeline_kind);
